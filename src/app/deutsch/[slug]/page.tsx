...
import { deutschTopics } from '@/lib/deutsch-data';
import H5PPlayer, { XAPIStatement } from '@/components/H5PPlayer';
import MathText from '@/components/MathText';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function DeutschDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const resolvedParams = use(params);
  const topic = deutschTopics[resolvedParams.slug];
    ...
    <div className="site-wrapper">
      ...
      <main className="content-container">
        ...
        <div className="breadcrumb-nav">
          ...
          <span>/</span>
          <Link href="/deutsch">Deutsch</Link>
          <span>/</span>
        ...
        {topic.keyPoints.length > 0 && (
          <div className="topic-rules-card">
            <h2>Wichtige Sprachregeln, Epochen-Merkmale und literarische Hintergründe</h2>
        ...
        <section className="info-box-section" style={{ marginTop: '40px' }}>
          <h3>Unterrichtsmaterialien & Lektürehilfen</h3>
          <p>
            Passend zu <strong>{topic.title}</strong> gibt es auf EDUKI vertiefende Kopiervorlagen, Textanalysen und Arbeitshefte zum Download.
          </p>
          <a
            href={topic.worksheetLink || `https://eduki.com/de/autor/1430402/about-the-world-org?query=${encodeURIComponent(topic.title)}+deutsch&t=284`}
            target="_blank"
