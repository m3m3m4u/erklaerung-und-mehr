...
import { geschichteTopics } from '@/lib/geschichte-data';
import H5PPlayer, { XAPIStatement } from '@/components/H5PPlayer';
import MathText from '@/components/MathText';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function GeschichteDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const resolvedParams = use(params);
  const topic = geschichteTopics[resolvedParams.slug];
      ...
      <main className="content-container">
        ...
        {topic.keyPoints.length > 0 && (
          <div className="topic-rules-card">
            <h2>Wichtige Daten, Fakten und Zusammenhänge</h2>
        ...
        {topic.worksheetLink && (
          <section className="info-box-section" style={{ marginTop: '40px' }}>
            ...
            <p>
              Möchtest du das Thema <strong>{topic.title}</strong> zusätzlich im Unterricht oder zu Hause auf Papier vertiefen? Auf EDUKI findest du passende Arbeitsblätter, Quellenmaterial und Tests zum Download.
            </p>
            <a
