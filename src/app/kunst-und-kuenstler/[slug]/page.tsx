...
import { kunstTopics } from '@/lib/kunst-data';
import H5PPlayer, { XAPIStatement } from '@/components/H5PPlayer';
import MathText from '@/components/MathText';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function KunstDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const resolvedParams = use(params);
  const topic = kunstTopics[resolvedParams.slug];
    ...
    <div className="site-wrapper">
      ...
      <main className="content-container">
        ...
        <div className="breadcrumb-nav">
          ...
          <span>/</span>
          <Link href="/kunst-und-kuenstler">Kunst und Künstler</Link>
          <span>/</span>
        ...
        {topic.keyPoints.length > 0 && (
          <div className="topic-rules-card">
            <h2>Wichtige Merkmale, Techniken und historische Hintergründe</h2>
        ...
        <section className="info-box-section" style={{ marginTop: '40px' }}>
          <h3>Unterrichtsmaterialien & Bildanalysen</h3>
          <p>
            Passend zu <strong>{topic.title}</strong> gibt es auf EDUKI vertiefende Kopiervorlagen, Zeichenanleitungen und Bildanalysen zum Download.
          </p>
          <a
            href={topic.worksheetLink || `https://eduki.com/de/autor/1430402/erklaerung-und-mehr-org?query=${encodeURIComponent(topic.title)}+kunst&t=284`}
            target="_blank"
