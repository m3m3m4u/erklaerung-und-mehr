...
import { mathTopics } from '@/lib/math-data';
import H5PPlayer, { XAPIStatement } from '@/components/H5PPlayer';
import MathText from '@/components/MathText';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function MathTopicDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const resolvedParams = use(params);
  const topic = mathTopics[resolvedParams.slug];
      ...
      <main className="content-container">
        ...
        {topic.keyPoints.length > 0 && (
          <div className="topic-rules-card">
            <h2>Wichtige Grundlagen und Regeln</h2>
        ...
        {topic.worksheetLink && (
          <section className="info-box-section" style={{ marginTop: '40px' }}>
            ...
            <p>
              Möchtest du das Thema <strong>{topic.title}</strong> zusätzlich auf Papier üben? Auf EDUKI findest du passende Arbeitsblätter und Tests zum Ausdrucken.
            </p>
            <a
