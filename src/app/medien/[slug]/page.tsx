...
import { medienTopics } from '@/lib/medien-data';
import H5PPlayer, { XAPIStatement } from '@/components/H5PPlayer';
import MathText from '@/components/MathText';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function MedienDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const resolvedParams = use(params);
  const topic = medienTopics[resolvedParams.slug];
      ...
      <main className="content-container">
        ...
        {topic.keyPoints.length > 0 && (
          <div className="topic-rules-card">
            <h2>Wichtige Medienkompetenzen, Hintergründe und Leitlinien</h2>
        ...
        <section className="info-box-section" style={{ marginTop: '40px' }}>
          <h3>Unterrichtsmaterialien & Medienkunde</h3>
          <p>
            Passend zu <strong>{topic.title}</strong> gibt es auf EDUKI vertiefende Kopiervorlagen, Stundenentwürfe und Diskussionsleitfäden zum Download.
          </p>
          <a
            href={topic.worksheetLink || `https://eduki.com/de/autor/1430402/about-the-world-org?query=${encodeURIComponent(topic.title)}+medien&t=1721`}
            target="_blank"
