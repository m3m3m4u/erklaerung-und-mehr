...
import { physikTopics } from '@/lib/physik-data';
import H5PPlayer, { XAPIStatement } from '@/components/H5PPlayer';
import MathText from '@/components/MathText';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function PhysikDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const resolvedParams = use(params);
  const topic = physikTopics[resolvedParams.slug];
  ...
  return (
    <div className="site-wrapper">
      ...
      <main className="content-container">
        ...
        {/* Key Rules & Formulas Box */}
        {topic.keyPoints.length > 0 && (
          <div className="topic-rules-card">
            <h2>Wichtige Gesetze, Formeln und Merksätze</h2>
        ...
        {topic.worksheetLink && (
          <section className="info-box-section" style={{ marginTop: '40px' }}>
            <h3>Passende Arbeitsblätter & Versuchsanleitungen</h3>
            <p>
              Möchtest du das Thema <strong>{topic.title}</strong> zusätzlich im Unterricht oder zu Hause auf Papier vertiefen? Auf EDUKI findest du passende Kopiervorlagen, Tests und Arbeitshefte zum Download.
            </p>
            <a
