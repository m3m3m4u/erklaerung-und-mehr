...
import { psychologieTopics } from '@/lib/psychologie-data';
import H5PPlayer, { XAPIStatement } from '@/components/H5PPlayer';
import MathText from '@/components/MathText';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function PsychologieDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const resolvedParams = use(params);
  const topic = psychologieTopics[resolvedParams.slug];
      ...
      <main className="content-container">
        ...
        {topic.keyPoints.length > 0 && (
          <div className="topic-rules-card">
            <h2>Wichtige Konzepte, Modelle und Forschungsergebnisse</h2>
        ...
        <section className="info-box-section" style={{ marginTop: '40px' }}>
          ...
          <p>
            Passend zu <strong>{topic.title}</strong> gibt es auf EDUKI vertiefende Kopiervorlagen, Fallbeispiele, Experiment-Protokolle und strukturierte Arbeitshefte.
          </p>
          <a
            href={topic.worksheetLink || `https://eduki.com/de/autor/1430402/about-the-world-org?query=${encodeURIComponent(topic.title)}+psychologie&t=3752`}
            target="_blank"
