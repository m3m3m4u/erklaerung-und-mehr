...
import { sozialesLernenTopics } from '@/lib/soziales-lernen-data';
import H5PPlayer, { XAPIStatement } from '@/components/H5PPlayer';
import MathText from '@/components/MathText';
import Header from '@/components/Header';
import Footer from '@/components/Footer';

export default function SozialesLernenDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const resolvedParams = use(params);
  const topic = sozialesLernenTopics[resolvedParams.slug];
    ...
    <div className="site-wrapper">
      ...
      <main className="content-container">
        ...
        <div className="breadcrumb-nav">
          ...
          <span>/</span>
          <Link href="/soziales-und-emotionales-lernen">Soziales & emotionales Lernen</Link>
          <span>/</span>
        ...
        {topic.keyPoints.length > 0 && (
          <div className="topic-rules-card">
            <h2>Wichtige Strategien, Tipps und Merksätze</h2>
        ...
        <section className="info-box-section" style={{ marginTop: '40px' }}>
          <h3>Unterrichtsmaterialien & Reflexionskarten</h3>
          <p>
            Passend zu <strong>{topic.title}</strong> gibt es auf EDUKI vertiefende Kopiervorlagen, Gefühlskarten, Rollenspiele und strukturierte Arbeitsblätter.
          </p>
          <a
            href={topic.worksheetLink || `https://eduki.com/de/autor/1430402/about-the-world-org?query=${encodeURIComponent(topic.title)}+soziales+lernen&t=3752`}
            target="_blank"
